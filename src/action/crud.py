...
class CrudView(MethodView):
    ...
    Register on initialization a service and a formatter.
    Request will be passed to the service through the CrudView.execute method
    and the result will be passed to the formatter.
    """
...
class CrudListView(CrudView):
    ...
    def post(self):
        attrs = request.form
        return self.execute('list', attrs)
...
class CrudListFieldView(CrudView):
    ...
    def post(self, field_name):
        attrs = request.form
        return self.execute('list_field', field_name, attrs)
...
def crud_actions(blueprint, resource_name, resource_service, formatter=None):
    ...
    blueprint.add_url_rule('/<resource_id>',
                           view_func=crud_get,
                           methods=['GET'])
    ...
    crud_get_full = CrudGetFullView.as_view(resource_name + '_get_full',
                                            resource_service=resource_service)
    blueprint.add_url_rule('/get_full/<resource_id>',
                           view_func=crud_get_full,
                           methods=['GET'])
    ...
    crud_list_field = CrudListFieldView.as_view(resource_name + '_list_field',
                                                resource_service=resource_service)
    blueprint.add_url_rule('/field_name/<field_name>',
                           view_func=crud_list_field,
                           methods=['POST'])
